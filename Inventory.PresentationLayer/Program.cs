    ...
    class Program
    {
        ...
        static async Task<(UserType, IUser)> ShowLoginScreen()
        {
            ...
            using (ISystemUserBL systemUserBL = new SystemUserBL())
            {
             ...
             {
                //Invoke GetAdminByEmailAndPasswordBL for checking email and password of Admin
                Supplier supplier = await supplierBL.GetSupplierByEmailAndPasswordBL(email, password);
                if (supplier != null)
                {
                    return (UserType.Supplier, supplier);
                }
             }
            ...
            {
                //Invoke GetAdminByEmailAndPasswordBL for checking email and password of Admin
                Distributor distributor = await distributorBL.GetDistributorByEmailAndPasswordBL(email, password);
                if (distributor != null)
                {
                    return (UserType.Distributor, distributor);
                }
            }
