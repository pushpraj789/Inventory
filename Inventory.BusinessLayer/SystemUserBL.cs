...
namespace Capgemini.Inventory.BusinessLayer
{
    ...
    {
        //fields
        private SystemUserDALBase systemUserDAL;
        ...
        protected async override Task<bool> Validate(SystemUser entityObject)
        {
            ...
            //Email is Unique
            var existingObject = await GetSystemUserByEmailBL(entityObject.Email);
            if (existingObject != null && existingObject?.SystemUserID != entityObject.SystemUserID)
            {
                valid = false;
